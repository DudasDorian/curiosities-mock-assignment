...
-- Diff Formatter
formatShowing :: [Showing String] -> String
formatShowing = formatHelper where
    formatHelper :: [Showing String] -> String
    formatHelper [] = ""
    formatHelper (x:xs) =
        case x of
            Normal a -> a ++ formatHelper xs
            Highlight a -> Color.highlight a ++ formatHelper xs

class Show e => Expectation e where
  holds :: e -> Bool

data EqualityAssertion where
  EqualityAssertion :: (Show a, Eq a, Diff a) => a -> a -> EqualityAssertion

shouldBe :: (Show a, Eq a, Diff a) => a -> a -> EqualityAssertion
...
instance Show EqualityAssertion where
  show (EqualityAssertion lhs rhs) = 
    printf "  Actual:   %s\n  Expected: %s" (formatShowing $ showDifference lhs rhs) (show rhs)
